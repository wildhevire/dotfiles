...
Singleton {
  ...
  property real nvidiaPercent: 0
  property string nvidiaDebug: ""
  ...
  Process {
    ...
    stdout: StdioCollector {
      onStreamFinished: {
        ...
        const dataParse = data[1].split(',')
        root.nvidiaDebug = dataParse[0]
        const data1 = dataParse[0]
        const data2 = dataParse[1]
        const temp = data1.replace(/[^0-9]/g, '')
        const percent = data2.replace(/[^0-9]/g, '') 
        root.nvidiaTemp = parseInt( parseInt(temp) || 0)
        root.nvidiaPercent = parseInt( parseInt(percent)|| 0)
      }
    }
  }
